...
def compute_standings(league):
    ...
    USER = ["R","HR","RBI","SB","AVG","OPS","K","QS","BAA","ERA","WHIP","SVHD"]
    REV = {"BAA","ERA","WHIP"}
    def tname(t):
        return (t.get("name") or " ".join(x for x in (t.get("location"), t.get("nickname")) if x)
                or f"Team {t.get('id')}").strip()
    ...
    for t in teams:
        ...
        tds.append({"team": tname(t), "vals": t.get("valuesByStat") or {},
                    "acq": tc.get("acquisitions") or 0, "drops": tc.get("drops") or 0,
                    "trades": tc.get("trades") or 0})
    total_moves = sum(td["acq"] for td in tds)
    cats = []
    if len(items) == len(USER):
        for i, it in enumerate(items):
            sid = it.get("statId"); label = USER[i]; rev = label in REV
            rows = []
            for td in tds:
            ...
            cats.append({"label": label, "statId": sid, "reverse": rev,
                         "apiReverse": bool(it.get("isReverseItem")), "top3": rows[:3]})
    diag = {"nItems": len(items), "nCats": len(cats), "nTeams": len(tds),
            "hasValues": any(td["vals"] for td in tds), "movesAcq": total_moves,
            "movesAll": sum(td["acq"]+td["drops"]+td["trades"] for td in tds),
            "reverseLabels": [c["label"] for c in cats if c["apiReverse"]]}
    # probe: for each scoring item, the top (hi) and bottom (lo) team+value, so the
    # label<->statId mapping can be corrected against known standings.
    probe = []
    for it in items:
        sid = it.get("statId"); vv = []
        for td in tds:
            v = td["vals"].get(str(sid))
            if v is None: v = td["vals"].get(sid)
            if v is not None:
                try: vv.append((td["team"], float(v)))
                except (TypeError, ValueError): pass
        if vv:
            hi = max(vv, key=lambda x: x[1]); lo = min(vv, key=lambda x: x[1])
            probe.append({"statId": sid, "rev": bool(it.get("isReverseItem")),
                          "hi": f"{hi[0]}={hi[1]:g}", "lo": f"{lo[0]}={lo[1]:g}"})
        else:
            probe.append({"statId": sid, "rev": bool(it.get("isReverseItem")), "hi": "-", "lo": "-"})
    diag["probe"] = probe
    return {"pool": total_moves, "totalMoves": total_moves, "categories": cats, "diag": diag}
...
def build():
    ...
    if USE_ESPN:
        ...
        try:
            ...
            print(f"  standings: {standings['diag']['nCats']} categories, "
